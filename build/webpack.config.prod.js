...
const postcssExtracter = new ExtractTextWebpackPlugin({
  filename: './css/[name]-postcss.[contenthash:8].css',  // 直接导入的css文件，提取时添加-css标识
  allChunks: true,  // 从所有的chunk中提取，当有CommonsChunkPlugin时，必须为true
});

const lessExtracter = new ExtractTextWebpackPlugin({
  filename: './css/[name]-less.[contenthash:8].css',  // 直接导入的css文件，提取时添加-css标识
  allChunks: true,  // 从所有的chunk中提取，当有CommonsChunkPlugin时，必须为true
});
...
const scssExtracter = new ExtractTextWebpackPlugin({
  filename: './css/[name]-scss.[contenthash:8].css', // 直接导入的sass文件，提取时添加-sass标识
  allChunks: true,  // 从所有的chunk中提取，当有CommonsChunkPlugin时，必须为true
});

const stylusExtracter = new ExtractTextWebpackPlugin({
  filename: './css/[name]-stylus.[contenthash:8].css',  // 直接导入的css文件，提取时添加-css标识
  allChunks: true,  // 从所有的chunk中提取，当有CommonsChunkPlugin时，必须为true
});

const stylExtracter = new ExtractTextWebpackPlugin({
  filename: './css/[name]-styl.[contenthash:8].css',  // 直接导入的css文件，提取时添加-css标识
  allChunks: true,  // 从所有的chunk中提取，当有CommonsChunkPlugin时，必须为true
});
...
const webpackProd = { // 生产配置文件
  output: {
    ...
    filename: 'js/[name].[chunkhash:8].bundle.js',     // 构建文件名
    chunkFilename: 'js/[id].[chunkhash：8].js',        // 按需加载的文件名
  },
  // 是否开启 sourceMap
